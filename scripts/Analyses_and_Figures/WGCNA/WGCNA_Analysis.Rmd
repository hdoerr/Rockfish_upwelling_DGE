...
datExpr = read.csv("Black_gill_normalized_counts_shortened.csv")
...
```{r}
datTraits = read.csv("Black_gill_phenotypic_new.csv")
head(datTraits)
...
cex.text = 0.9,
zlim = c(-.8,.8),
main = paste("Black Module-Trait correlations"))
dev.off()
```
...
weight = as.data.frame(datTraits$Ucrit_relative); #change to your trait name of interest
names(weight) = "Ucrit_relative"
...
moduleCols=c( "turquoise","orangered4","antiquewhite4", "skyblue3", "lightcyan1") 
par(mfrow=c(1,4))
...
```{r}
annot=read.csv("Black_Trinity_FR_longest_iso.swissprot.csv",header=TRUE,sep=",") #past_apr2014_iso2gene.tab; kb8_mar2014_iso2gene.tab
...
write.table(datOutput,"AnnotatedNetworkAnalysisResults1k_rlog_Black_Merge40.csv",row.names=F,sep=",") 
```

5. Now you can save files containing the gene names and uniprot ID's for each interesting module.

```{r}
#Change the module color to create and save each file in the code below. Interesting modules for copgil "turquoise","orangered4","antiquewhite4", "skyblue3", "lightcyan1"

#Get the corresponding uniprot id's
allUniprotIDs=annot$Uniprot_Swpt_ID[probes2annot];

#Get the gene names
modGenes = names(datExpr)[moduleColors=="turquoise"]
#Get their uniprot ID's
modUniprotIDs=allUniprotIDs[moduleColors=="turquoise"];
#Write to a new file and save
fileName = data.frame(modGenes, modUniprotIDs)
write.table(fileName, "turquoise_Module_genes_uniprot.csv", row.names=F, sep=",")
