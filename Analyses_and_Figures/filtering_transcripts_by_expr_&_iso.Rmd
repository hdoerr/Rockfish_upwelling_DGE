...
```{r}
perl /projects/rockfish/transcriptomics/bake1349/trinityrnaseq-Trinity-v2.4.0/util/filter_low_expr_transcripts.pl --matrix matrix.TMM.EXPR.matrix --transcripts /projects/rockfish/transcriptomics/EXP11_ScarnSmel_fluct_gill/Copgil_mapping/Copgil_HMD_Trinity_FR.fasta --highest_iso_only --trinity_mode > Copper_normalized_filtered.txt
...
```{r}
sed 's/_i../ /' Copper_normalized_filtered.txt > Copgil_normalized_filtered_no_ix.txt #This commands searches through the file for the characters "_i" and anything that comes immediately after it (before spaces), and replaces it with a space.
```

B. Check that you retained the correct number of transcripts by computing the 'TRINITY' count: 
```{r}
awk '/TRINITY*/ ' Copgil_normalized_filtered.txt | wc -l
          -> 420348
awk '/TRINITY*/ ' Copgil_normalized_filtered_no_ix.txt | wc -l
          -> 420348
```
...
```{r}
perl /projects/rockfish/transcriptomics/bake1349/trinityrnaseq-Trinity-v2.4.0/util/filter_low_expr_transcripts.pl --matrix matrix.TMM.EXPR.matrix --transcripts Copgil_normalized_filtered_no_ix.txt --min_expr_any 6 --trinity_mode > Copgil_normalized_filtered_expr_6.txt
    #You can play around with the expression threshold, i.e. 0.5 vs 1
    # ->  Retained 98432 / 420348 = 23.42% of total transcripts.
```

At this point you will likely want to pull out just the contig names to create a new list, then open in excel and save as a .csv. You can easily pull out the contig names by using the following on the command line:
```{r}
awk '/TRINITY*/ ' Copgil_normalized_filtered_expr_6.txt > Copgil_norm_filt_trans_list_6.txt #finds each 'TRINITY' line and prints it to a new file

awk '{print $1}' Copgil_norm_filt_trans_list_6.txt > Copgil_norm_filt_trans_6_list.txt #searches the new file to pull the first column (in this case the contig ID we want) and print it to a new file 
```


Now that we have our fully filtered transcript list, we can use R to merge the filtered list with our original TMM.matrix to obtain corresponding expression values to use for our WGCNA Analysis
...
First, open RStudio and read in your .csv files:
...
setwd("C:/Users/hdoer/Desktop/thesis/new/data/DGE/Copper_gill/DGE_sans_4_samps/WGCNA/")

exprlist <- read.csv("C:/Users/hdoer/Desktop/thesis/new/data/DGE/Copper_gill/DGE_sans_4_samps/WGCNA/Copgill_normalized_counts.csv")

filterlist <- read.csv("C:/Users/hdoer/Desktop/thesis/new/data/DGE/Gopher_gill/Goph_map_to_copgil/WGCNA/Copgil_norm_filt_trans_6_list.csv")
...
write_csv(merged, "Copgil_norm_expr_6_filt.csv")

```

From here you can continue on to your WGCNA analysis! 

Find further WGCNA instructions here: 
